...
<?php if ( $show_account ) {
    echo '<a class="header-item wc-account-link" href="' . esc_url( get_permalink( get_option('woocommerce_myaccount_page_id') ) ) . '" title="' . esc_html__( 'Your account', 'botiga' ) . '"><i class="ws-svg-icon">' . botiga_get_header_icon( 'account' ) . '</i>Login</a>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
} ?>	
